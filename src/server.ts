...
import { CustomError, IErrorResponse, winstonLogger } from '@l3ernardo/jobber-shared';
...
//declares a class named GatewayServer, that can be export keyword allows this class to be imported and used in other files.
export class GatewayServer {
  private app: Application; // The private keyword means that this property can only be accessed within the GatewayServer class itself.

  constructor(app: Application) { // The constructor is a special method that is called when a new instance of the class is created.
    this.app = app;
  }
  ...
  private securityMiddleware(app: Application): void {
    ...
    app.use(hpp()); // HTTP Parameter Pollution (HPP) is a type of attack where an attacker manipulates the URL or query parameters to include multiple parameters with the same name.
    app.use(helmet()); //Helmet is a security-focused middleware for Express that sets various HTTP headers
    app.use(cors({
